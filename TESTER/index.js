...
function attemptFetches(limit) {
    ...
    let promises = [...users].map((user) => {
        ...
        return async () => {
            await fetch(`http://${HOST}:${PORT}/users/${user}/cards`, {signal: Timeout(20).signal})
            .then(res => res.json())
            .then(_ => ++successFetchCount)
    ...
    const promise = new Promise(async (resolve, reject) => {
        ...
        async.parallelLimit(promises, limit).then(() => {
...
function createParentWS(resolve, retrieveCount, sockets) {
    ...
    client.on('close', async () => {
        ...
        if(sockets().filter(socket => socket.readyState !== ws.CLOSED).length > 0) {
...
function attemptClockFetches(clientCount, retrieveCount) {
    ...
    for(let i = 0; i < clientCount - 1; ++i) {
        clients.push(async () => new Promise(wsFunction(i + 1, retrieveCount, Math.max(clientCount * 10, 500))));
    }
    ...
    let promise = new Promise(async resolve => {
        ...
        client.on('open', async () => {
            ...
            sockets = sockets.filter(value => !!value && value.readyState == ws.OPEN);
...
function appendResult(output, type, count, result, errorCount) {
    if(!Object.keys(output[type]).includes(count)) {
        output[type][count] = {success: [], error: []};
    }
...
async function doStressTest() {
    ...
    if(process.argv.length === 3) {
        ...
        console.log("Tested server unknown, please retry with 'npm run start <env>'");
        return;
    }
    ...
    for(let repeat = 0; repeat < repeatCount; ++repeat) {
        ...
        if(!skipREST) {
            ...
            console.log("Warmups complete, starting real tests: 4096 fetches in specified group sizes");
            ...
            for(let i = 1; i <= 4096; i *= 2) {
                ...
                try {
                    errorCount = 0;
                    let result = await attemptFetches(i);
                    appendResult(results, "fetch", i, result, 4096 - successFetchCount);
                    console.log(`${i}, ${result}, ${4096 - successFetchCount}`);
                    socketSet.clear();
                } catch(err) {
        ...
        if(!skipWebsockets) {
            ...
            for(let i = 1; i <= 4096; i += i) {
                ...
                try {
                    ...
                    if(result) {
                        appendResult(results, "ws", i, result, i - successFetchCount);
                        console.log(`${i}, ${result}, ${i - successFetchCount}, SUCCESS: ${successFetchCount}`);
                        errorCount = 0;
                    } else {
    ...
    let writeStream = "type,repeatCount,testRepeatCount,testRepeatAverage,errors";
    for(let rootKey of Object.keys(results)) {
        for(let count of Object.keys(results[rootKey])) {
            writeStream += "\n" + [rootKey, count, repeatCount, average(results[rootKey][count].success), average(results[rootKey][count].error)].join(",");
        }
    }

    writeStream += "\n";
    fs.writeFile(testServer + "_" + performance.now() + ".csv", writeStream, err => {
